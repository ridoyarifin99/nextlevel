...
function getMimeType(imagePath) {
    if (/\.(jpg|jpeg)$/i.test(imagePath)) return "image/jpeg";
    if (/\.webp$/i.test(imagePath)) return "image/webp";
    if (/\.avif$/i.test(imagePath)) return "image/avif";
    return "image/png";
}
...
function resolveProductKey(value) {
    ...
    if (productIndex[normalized]) return productIndex[normalized];

    const generated = productSlug(normalized);
    if (productIndex[generated]) return productIndex[generated];
...
<body>
    ...
    <p><a href="${escapeHTML(SITE.domain)}/">Return to NEXT LEVEL SUBS</a></p>
</body>
</html>`);
}
...
module.exports = function handler(req, res) {
    ...
    const description = `${product.description}. Get ${product.name} subscription from NEXT LEVEL SUBS.`;
    const imageAlt = `${product.name} - NEXT LEVEL SUBS`;
...
<body>
    <iframe id="productFrame" src="${escapeHTML(destinationURL)}" title="${escapeHTML(product.name)}" loading="eager" allow="fullscreen"></iframe>

    <noscript>
        <div class="fallback">
            <a href="${escapeHTML(destinationURL)}">Continue to ${escapeHTML(product.name)}</a>
        </div>
    </noscript>
