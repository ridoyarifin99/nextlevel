...
values
('Streaming','streaming','Movies, TV, live and on-demand entertainment.',10),
('Music','music','Music and audio streaming services.',20),
('VPN & Security','vpn-security','VPN, privacy and security services.',30),
('AI & Productivity','ai-productivity','AI, productivity and creative tools.',40),
('Cloud Storage','cloud-storage','Cloud storage and backup services.',50),
('Software & Apps','software-apps','Software, utilities and premium apps.',60),
('Education','education','Learning, courses and education services.',70),
('Gaming','gaming','Gaming subscriptions and services.',80),
('Bundles & Combos','bundles-combos','Multi-service bundles and combo offers.',90),
('Social & Communication','social-communication','Communication and social utility services.',100),
('Adult Entertainment','adult-entertainment','Age-restricted entertainment services.',110),
('Other Digital Services','other-digital-services','Digital services that do not fit another category.',999)
...
create policy plans_public_read on public.product_plans for select using (
  is_available = true and exists (
    select 1 from public.products p where p.id = product_plans.product_id and p.is_available = true and p.is_archived = false
  )
);
...
-- Add the authenticated admin once after running this script:
