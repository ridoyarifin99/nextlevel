...
(() => {
  ...
  function styles(){
    ...
    const s=document.createElement("style");s.id="nls-premium-content-styles";s.textContent=`
      .nls-premium-tab{animation:nlsPremiumIn .55s cubic-bezier(.16,1,.3,1) both}
      @keyframes nlsPremiumIn{from{opacity:0;transform:translateY(14px);filter:blur(3px)}to{opacity:1;transform:none;filter:none}}
      .nls-premium-overview{position:relative;overflow:hidden;border:1px solid rgba(124,58,237,.12);border-radius:24px;padding:28px;background:radial-gradient(circle at 100% 0,rgba(37,117,252,.10),transparent 34%),radial-gradient(circle at 0 100%,rgba(106,17,203,.10),transparent 36%),linear-gradient(145deg,#fff,#faf9ff);box-shadow:0 20px 55px rgba(76,29,149,.08)}
      ...
      .nls-premium-eyebrow{display:inline-flex;align-items:center;gap:7px;padding:6px 10px;border-radius:999px;background:rgba(106,17,203,.08);color:#6d28d9;font-size:11px;font-weight:900;letter-spacing:.08em;text-transform:uppercase}
      .nls-premium-overview h3{margin:15px 0 9px;font-size:24px;line-height:1.2;color:#111827;font-weight:900;letter-spacing:-.025em}
      .nls-premium-overview p{margin:0;max-width:850px;color:#475569;font-size:15px;line-height:1.9}
      ...
      .nls-feature-head,.nls-faq-head{display:flex;align-items:end;justify-content:space-between;gap:15px;margin-bottom:16px}.nls-feature-head h3,.nls-faq-head h3{margin:0;color:#111827;font-size:23px;font-weight:900;letter-spacing:-.025em}.nls-feature-head p,.nls-faq-head p{margin:5px 0 0;color:#64748b;font-size:13px}
      .nls-feature-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:12px}
      .nls-feature-card{position:relative;display:flex;gap:13px;align-items:flex-start;padding:17px;border:1px solid #e9e7f1;border-radius:17px;background:linear-gradient(145deg,#fff,#fbfaff);box-shadow:0 8px 25px rgba(15,23,42,.045);opacity:0;transform:translateY(14px);animation:nlsFeatureIn .55s cubic-bezier(.16,1,.3,1) forwards;transition:transform .3s cubic-bezier(.16,1,.3,1),box-shadow .3s,border-color .3s}
      ...
      .nls-feature-index{position:absolute;right:12px;top:10px;font-size:10px;font-weight:900;color:#c4b5fd}.nls-feature-card strong{position:relative;z-index:1;color:#334155;font-size:14px;line-height:1.55;font-weight:750}
      ...
      .nls-faq-q{width:100%;display:flex;align-items:center;gap:12px;text-align:left;border:0;background:transparent;padding:18px 19px;cursor:pointer;color:#1f2937;font-weight:850;font-size:14px}.nls-faq-num{width:28px;height:28px;display:grid;place-items:center;border-radius:9px;background:#f3e8ff;color:#7c3aed;font-size:10px;font-weight:900;flex:0 0 28px}.nls-faq-chevron{margin-left:auto;width:30px;height:30px;border-radius:50%;display:grid;place-items:center;background:#f8fafc;color:#64748b;transition:transform .35s,background .35s,color .35s}.nls-faq-item.open .nls-faq-chevron{transform:rotate(180deg);background:#ede9fe;color:#6d28d9}.nls-faq-a{display:grid;grid-template-rows:0fr;transition:grid-template-rows .42s cubic-bezier(.16,1,.3,1)}.nls-faq-item.open .nls-faq-a{grid-template-rows:1fr}.nls-faq-a>div{overflow:hidden}.nls-faq-answer{padding:0 19px 18px 59px;color:#64748b;font-size:13px;line-height:1.85}
      .nls-faq-search{position:relative;max-width:290px;flex:1}.nls-faq-search input{width:100%;border:1px solid #e2e0e8;border-radius:12px;padding:10px 12px 10px 36px;outline:0;font-size:12px;background:#fff;transition:.25s}.nls-faq-search input:focus{border-color:#a78bfa;box-shadow:0 0 0 4px rgba(139,92,246,.08)}.nls-faq-search i{position:absolute;left:13px;top:50%;transform:translateY(-50%);color:#94a3b8;font-size:12px}.nls-faq-empty{text-align:center;padding:35px 15px;border:1px dashed #ddd6fe;border-radius:17px;color:#64748b;background:#faf9ff}
      @media(max-width:700px){.nls-feature-grid{grid-template-columns:1fr}.nls-premium-overview{padding:22px;border-radius:20px}.nls-premium-overview h3{font-size:21px}.nls-feature-head,.nls-faq-head{align-items:stretch;flex-direction:column}.nls-faq-search{max-width:none}.nls-faq-answer{padding-left:59px}}
      @media(prefers-reduced-motion:reduce){.nls-premium-tab,.nls-feature-card{animation:none;opacity:1;transform:none}.nls-faq-a,.nls-faq-chevron{transition:none}}
  ...
  function premiumDescription(){
    const p=window.currentProduct||{},d=String(p.description||"").trim();
    const planCount=Array.isArray(p.pricing)?p.pricing.length:0;
  ...
  function install(){
    ...
    setTimeout(()=>{const active=document.querySelector(".tab-button.active")?.textContent?.trim().toLowerCase();enhanceTab(active?.startsWith("feature")?"features":active?.startsWith("faq")?"faq":"description")},150);
  }
