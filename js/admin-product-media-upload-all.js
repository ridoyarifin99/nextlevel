...
(() => {
  ...
  async function listAll(prefix){
    ...
    async function walk(folder){
      let offset=0;
      for(;;){
        const {data,error}=await db().storage.from(BUCKET).list(folder,{limit:100,offset,sortBy:{column:"name",order:"asc"}});if(error)throw error;
        const items=data||[];for(const item of items){const name=item?.name;if(!name)continue;const path=folder?`${folder}/${name}`:name;if(item.id)out.push(path);else await walk(path)}
        if(items.length<100)break;offset+=items.length;
      }
    }
    await walk(prefix);return out;
  }
  ...
  async function run(){
    if(!confirm("REPAIR all product images? This will remove the current centralized product-media records/files and rebuild primary, details gallery and service images from the existing website catalog. This fixes previous duplicate-image uploads."))return;
    ...
    try{
      ...
      const {data:products,error}=await db().from("products").select("id,slug,name,image_url,icon").order("display_order");if(error)throw error;
      let uploaded=0,failed=0,removedFiles=0;
      for(let i=0;i<products.length;i++){
        const p=products[i],slug=slugify(p.slug||p.name),old=legacyMap.get(String(p.slug).toLowerCase())||{},status=`${i+1}/${products.length} — ${p.name}`;setProgress(`Repairing ${status}`);
        try{
          const sourcePrimary=old.image||p.image_url||"";
          const sourceLogo=old.logo||"";
          ...
          await clearProductRows(p.id);
          removedFiles+=await clearProductStorage(slug);
          const rows=[];
          if(sourcePrimary){const primary=await uploadUrl(sourcePrimary,slug,"primary",0);if(primary)rows.push({product_id:p.id,role:"primary",url:primary.url,storage_path:primary.storage_path,alt_text:p.name,display_order:0,is_active:true,metadata:{source:"legacy_catalog_repair"}})}
          if(sourceLogo&&sourceLogo!==sourcePrimary){const logo=await uploadUrl(sourceLogo,slug,"logo",0);if(logo)rows.push({product_id:p.id,role:"logo",url:logo.url,storage_path:logo.storage_path,alt_text:`${p.name} logo`,display_order:0,is_active:true,metadata:{source:"legacy_catalog_repair"}})}
          for(let j=0;j<gallerySources.length;j++){const gallery=await uploadUrl(gallerySources[j],slug,"gallery",j);if(gallery)rows.push({product_id:p.id,role:"gallery",url:gallery.url,storage_path:gallery.storage_path,alt_text:`${p.name} ${j+1}`,display_order:j,is_active:true,metadata:{source:"legacy_catalog_repair"}})}
          for(let j=0;j<serviceSources.length;j++){const item=serviceSources[j],service=await uploadUrl(item.url,slug,"service",j);if(service)rows.push({product_id:p.id,role:"service",url:service.url,storage_path:service.storage_path,service_name:item.name||item.service_name||"",title:item.title||"",alt_text:item.alt||item.alt_text||p.name,display_order:j,is_active:true,metadata:{source:"legacy_catalog_repair"}})}
      ...
      setProgress(`Complete — ${uploaded} media records rebuilt, ${removedFiles} old storage files removed${failed?`; ${failed} products need attention`:""}.`);
      toast(`Product media repair complete. ${uploaded} media records rebuilt${failed?` with ${failed} product errors`:""}.`,failed>0);
  ...
  function boot(){
    ...
    const button=document.createElement("button");button.id="uploadAllProductMedia";button.className="btn";button.type="button";button.innerHTML='<i class="fas fa-cloud-arrow-up"></i> Repair & Upload All Images';button.title="Rebuild centralized product media from the current website catalog";button.onclick=run;
    const status=document.createElement("span");status.id="nlsMediaMigrationProgress";status.style.cssText="display:inline-flex;align-items:center;max-width:360px;color:#64748b;font-size:11px;font-weight:700";actions.insertBefore(button,actions.firstChild);actions.insertBefore(status,button.nextSibling);
  }
