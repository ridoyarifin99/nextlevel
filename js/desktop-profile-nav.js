(function () {
  ...
  async function profile(user) {
    try {
      const { data } = await window.supabaseClient.from("profiles")
        .select("avatar_url,full_name,phone,email").eq("id", user.id).maybeSingle();
      return data || {};
    } catch (_) { return {}; }
  }
  ...
  function styles() {
    ...
    s.textContent = `
      @media (min-width:1025px) {
        /* The existing account link remains the ONLY desktop profile button. */
        ...
        #userAccountArea:hover .nls-account-chevron,
        #userAccountArea:focus-within .nls-account-chevron { transform:rotate(180deg); }
        ...
        #userAccountArea:hover #nlsAccountDropdown,
        #userAccountArea:focus-within #nlsAccountDropdown {
          opacity:1;
          visibility:visible;
          pointer-events:auto;
          transform:translateY(0) scale(1);
        }
        #nlsAccountDropdown::before {
          content:"";
          position:absolute;
          top:-7px;
          left:0;
        ...
        #nlsAccountDropdown a,
        #nlsAccountDropdown button {
          width:100%;
          display:flex;
          align-items:center;
          gap:10px;
        ...
        #nlsAccountDropdown a:hover,
        #nlsAccountDropdown button:hover {
          color:#6a11cb;
          background:rgba(106,17,203,.08);
  ...
  function render(user, p) {
    const account = document.getElementById("userAccountArea");
    if (!account) return;
    if (!user) { account.style.display = "none"; return; }

    const m = user.user_metadata || {};
    const name = p.full_name || m.full_name || m.name || user.email || "User";
    const avatar = p.avatar_url || m.avatar_url || m.picture || m.photo_url || "";
    ...
    if (!dropdown) {
      ...
      dropdown.innerHTML = `
        <a href="/dashboard.html"><i class="fa-solid fa-gauge-high"></i><span>Dashboard</span></a>
        <button type="button" class="nls-dropdown-logout"><i class="fa-solid fa-right-from-bracket"></i><span>Logout</span></button>
      `;
      ...
      dropdown.querySelector(".nls-dropdown-logout").addEventListener("click", async e => {
        ...
        try {
          ...
          else await window.supabaseClient.auth.signOut();
  ...
  async function init() {
    ...
    if (data?.user) render(data.user, await profile(data.user));
    else document.getElementById("userAccountArea")?.style.setProperty("display","none","important");
    window.supabaseClient.auth.onAuthStateChange(async (event, session) => {
      if (!session?.user) {
        ...
        const l=document.getElementById("loginLink"); if(l) l.classList.remove("nls-login-hidden");
        return;
      }
      render(session.user, await profile(session.user));
    });
  }
