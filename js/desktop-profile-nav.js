(function () {
  ...
  function styles() {
    ...
    s.textContent = `
      @media (min-width:1025px) {
        /* Desktop profile menu: click/tap only. No hover-to-open behavior. */
        ...
        #loginLink.nls-account-link:hover{color:#334155;background:transparent;transform:none}
        #loginLink.nls-account-link:focus-visible{outline:2px solid rgba(106,17,203,.35);outline-offset:2px}
        ...
        #loginLink .nls-account-chevron{font-size:10px;transition:transform .25s ease}
        #loginLink.nls-account-open .nls-account-chevron{transform:rotate(180deg)}
        ...
        #loginLink.nls-account-open #nlsAccountDropdown{opacity:1;visibility:visible;pointer-events:auto;transform:translateY(0) scale(1)}
  ...
  function closeDropdown(account) {
    account?.classList.remove("nls-account-open");
  ...
  function render(data) {
    ...
    account.onclick = function (event) {
      ...
      const open = account.classList.contains("nls-account-open");
      closeDropdown(account);
      if (!open) {
        account.classList.add("nls-account-open");
  ...
  function bindGlobalClose() {
    if (window.__NLSDesktopProfileGlobalCloseBound) return;
    window.__NLSDesktopProfileGlobalCloseBound = true;
    document.addEventListener("click", event => {
      if (window.innerWidth < 1025) return;
      const account = document.getElementById("loginLink");
      if (account && !account.contains(event.target)) closeDropdown(account);
    });
    document.addEventListener("keydown", event => {
      if (event.key !== "Escape") return;
      closeDropdown(document.getElementById("loginLink"));
    });
    window.addEventListener("resize", () => {
      if (window.innerWidth < 1025) closeDropdown(document.getElementById("loginLink"));
    }, { passive: true });
  }
