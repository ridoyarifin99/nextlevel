(function () {
    ...
    function injectStyles() {
        ...
        style.id = STYLE_ID;
        style.textContent = `
            #${NAV_ID}{--nls-bn-primary:#6a11cb;--nls-bn-secondary:#2575fc;--nls-bn-text:#64748b;--nls-bn-active:#fff;position:fixed;left:10px;right:10px;bottom:max(10px,env(safe-area-inset-bottom));z-index:2147483000;display:none;height:70px;padding:7px;border:1px solid rgba(226,232,240,.82);border-radius:22px;background:rgba(255,255,255,.82);backdrop-filter:blur(24px) saturate(180%);-webkit-backdrop-filter:blur(24px) saturate(180%);box-shadow:0 18px 55px rgba(15,23,42,.18),0 3px 12px rgba(106,17,203,.08);isolation:isolate;overflow:visible;transform:translateY(120%);opacity:0;transition:transform .65s cubic-bezier(.16,1,.3,1),opacity .45s ease}
            #${NAV_ID}.nls-bn-ready{transform:translateY(0);opacity:1}
            #${NAV_ID} .nls-bn-track{position:relative;width:100%;height:100%;display:grid;grid-template-columns:repeat(4,minmax(0,1fr));align-items:stretch}
            #${NAV_ID} .nls-bn-slider{position:absolute;z-index:0;top:0;bottom:0;left:0;width:25%;border-radius:17px;background:linear-gradient(135deg,var(--nls-bn-primary),var(--nls-bn-secondary));box-shadow:0 8px 22px rgba(106,17,203,.24);transform:translateX(0);transition:transform .5s cubic-bezier(.16,1,.3,1);pointer-events:none}
            ...
            @media(max-width:1024px){body{padding-bottom:92px!important}#${NAV_ID}{display:block}}@media(max-width:430px){#${NAV_ID}{left:8px;right:8px;height:68px;border-radius:21px}#${NAV_ID} .nls-bn-label{font-size:9.5px}}@media(min-width:1025px){#${NAV_ID}{display:none!important}}@media(prefers-reduced-motion:reduce){#${NAV_ID},#${NAV_ID} .nls-bn-slider,#${NAV_ID} .nls-bn-item,#${NAV_ID} .nls-bn-item i,#${NAV_ID} .nls-bn-item .nls-bn-avatar,#${NAV_ID} .nls-bn-item .nls-bn-label{transition-duration:.01ms!important;animation-duration:.01ms!important}}
        `;
    ...
    function init(){if(!document.body)return;injectStyles();const nav=createNav();setActive(nav);updateAccountButton(nav);requestAnimationFrame(()=>requestAnimationFrame(()=>nav.classList.add('nls-bn-ready')));window.addEventListener('popstate',()=>setActive(nav));window.addEventListener('pageshow',()=>{setActive(nav);updateAccountButton(nav)});if(window.supabaseClient?.auth?.onAuthStateChange)window.supabaseClient.auth.onAuthStateChange(()=>updateAccountButton(nav));}
