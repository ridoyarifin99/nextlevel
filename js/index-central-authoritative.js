...
(function () {
  ...
  const categoryKey = v => norm(v).replace(/[&+\-\s]+/g, "").replace(/_/g, "");
  ...
  function categoriesOf(p){
    ...
    if(Array.isArray(p?.extra_data?.homepage_categories))out.push(...p.extra_data.homepage_categories);
    if(Array.isArray(p?.extra_data?.legacy_categories))out.push(...p.extra_data.legacy_categories);
  ...
  function categoryMatches(p,category){
    const keys=categoriesOf(p).map(categoryKey);
    const target=categoryKey(category);
    if(target==="aidesign")return keys.includes("aidesign")||keys.includes("aidesignservices")||keys.includes("aidigitaldesign");
    if(target==="combos")return keys.includes("combo")||keys.includes("combos");
  ...
  function makeCard(p,index,priority){
    const card=document.createElement("div");card.className="subscription-card";card.dataset.productId=p.id||"";card.dataset.productSlug=p.slug||"";card.dataset.centralAuthoritative="1";card.dataset.centralSignature=signature(p);card.dataset.aos="fade-up";card.dataset.aosDuration="450";card.dataset.aosDelay=String(Math.min(Number(index||0)*35,175));
    const image=imageOf(p),name=p.name,description=p.description||"",price=priceOf(p),duration=durationOf(p),cats=categoriesOf(p),best=cats.some(x=>categoryKey(x)==="bestselling")||!!p.is_featured,combo=cats.some(x=>categoryKey(x)==="combo"||categoryKey(x)==="combos");
    const loading=priority?"eager":"lazy",fetchPriority=priority?"high":"auto";
    const media=image?`<img src="${esc(image)}" alt="${esc(name)} logo" class="product-image" loading="${loading}" fetchpriority="${fetchPriority}" decoding="async">`:`<i class="${esc(p.icon||"fas fa-play-circle")} product-icon" aria-hidden="true"></i>`;
  ...
  function decorate(card,p,index,priority){
    card.dataset.productId=p.id||"";card.dataset.productSlug=p.slug||"";card.dataset.centralSignature=signature(p);card.dataset.aos="fade-up";card.dataset.aosDuration="450";card.dataset.aosDelay=String(Math.min(Number(index||0)*35,175));
    const img=card.querySelector("img"),image=imageOf(p);if(img&&image){img.src=image;img.removeAttribute("srcset");img.alt=`${p.name} logo`;img.loading=priority?"eager":"lazy";img.fetchPriority=priority?"high":"auto";}
  ...
  function ensureAOS(){
    ...
    if(!window.__NLSAOSInitialized){
      ...
      window.AOS.init({duration:450,easing:"ease-out-cubic",once:true,offset:60,delay:0,throttleDelay:120,debounceDelay:80,disableMutationObserver:true,disable:()=>window.matchMedia?.("(prefers-reduced-motion: reduce)").matches});
    }
    clearTimeout(aosTimer);
    aosTimer=setTimeout(()=>{if(window.AOS.refreshHard)window.AOS.refreshHard();else if(window.AOS.refresh)window.AOS.refresh();},50);
  }
  ...
  function renderCatalog(){
    ...
    Object.entries(CATEGORIES).forEach(([category,id])=>{
      ...
      const list=products.filter(p=>categoryMatches(p,category)||(category==="best-selling"&&p.is_featured));
      const listSignature=list.map(signature).join("||");
      if(container.dataset.nlsRenderedSignature===listSignature){container.removeAttribute("data-nls-central-pending");return;}
      ...
      const fragment=document.createDocumentFragment();list.forEach((p,index)=>{const priority=firstSection&&index<4;let card=reusable.get(String(p.id||""))||reusable.get(norm(p.slug));if(!card)card=makeCard(p,index,priority);else decorate(card,p,index,priority);card.dataset.centralAuthoritative="1";card.style.order=String(index);fragment.appendChild(card);});
      ...
      container.removeAttribute("data-nls-central-pending");container.dataset.nlsCentralCount=String(list.length);container.dataset.nlsRenderedSignature=listSignature;
      firstSection=false;
    });
  ...
  function boot(){window.addEventListener("nls:central-catalog-ready",apply);window.addEventListener("nextlevel:products-updated",apply);apply();setInterval(apply,30000);}
