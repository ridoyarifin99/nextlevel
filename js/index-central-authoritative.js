...
(function () {
  ...
  /* Hide legacy/placeholder cards until the central catalog is actually ready. */
  ...
  function validProduct(p) {
    return !!p && p.is_active !== false && !!String(p.slug ?? "").trim() && !!String(p.name ?? "").trim() && plansOf(p).length > 0;
  }

  function planOf(p) { return plansOf(p)[0]; }
  ...
  function cardForProduct(container, p) {
    return [...container.children].find(el => String(el.dataset?.productId || "") === String(p.id || "") || norm(el.dataset?.productSlug) === norm(p.slug));
  }
  ...
  function makeCard(p) {
    ...
    card.querySelector("[data-central-add-cart]")?.addEventListener("click", e => {
      ...
      window.NLSCentralAddToCart?.(p.slug);
    });
    return card;
  }
  ...
  function decorate(card, p) {
    ...
    if (link) link.href = `/product/${encodeURIComponent(p.slug)}`;
  ...
  function renderCatalog() {
    ...
    Object.entries(CATEGORIES).forEach(([category, id]) => {
      ...
      managed.forEach(card => reusable.set(String(card.dataset.productId || card.dataset.productSlug), card));
      const fragment = document.createDocumentFragment();

      list.forEach((p, index) => {
        let card = reusable.get(String(p.id || "")) || reusable.get(String(p.slug || ""));
      ...
      /* Delete every old/empty/legacy product card from this managed section. */
  ...
  function syncCart() {
    try {
      ...
      const raw = JSON.parse(localStorage.getItem("streamHubCart") || "[]");
      if (!Array.isArray(raw)) return;
      const next = raw.map(item => {
        const p = bySlug.get(norm(item.slug || item.product_slug));
        if (!p) return null;
        const plans = plansOf(p);
        const plan = plans.find(x => String(x.id) === String(item.selectedPlan?.id)) || plans.find(x => norm(x.duration) === norm(item.duration)) || plans[0];
        if (!plan) return null;
  ...
  let applying = false;
  function apply() {
    if (applying || !Array.isArray(window.NLSCentralCatalog?.products)) return;
    applying = true;
    try { renderCatalog(); syncCart(); } finally { applying = false; }
  }
  ...
  function boot() {
    ...
    apply();
    /* Safety only; realtime/event updates remain primary. */
