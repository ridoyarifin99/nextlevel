...
(() => {
  ...
  function ensureStyles(){
    ...
    const s=document.createElement("style");s.id="nls-existing-review-styles";s.textContent=`
      .nls-review-card{position:relative;border:1px solid #edf0f5;border-radius:18px;padding:18px;background:linear-gradient(180deg,#fff 0%,#fcfcff 100%);box-shadow:0 7px 24px rgba(15,23,42,.045);transition:transform .25s ease,box-shadow .25s ease,border-color .25s ease}
      .nls-review-card:hover{transform:translateY(-1px);box-shadow:0 12px 30px rgba(15,23,42,.07);border-color:#e7ddf8}
      .nls-review-avatar{width:44px;height:44px;border-radius:50%;object-fit:cover;display:block;flex:0 0 auto;border:2px solid #fff;box-shadow:0 2px 9px rgba(15,23,42,.12);background:linear-gradient(135deg,#ede9fe,#dbeafe)}
      ...
      .nls-existing-verified{display:inline-flex;align-items:center;gap:4px;margin-left:7px;font-size:11px;font-weight:800;color:#047857;background:#ecfdf5;padding:3px 8px;border-radius:99px}
      .nls-existing-reply-box{display:flex;gap:8px;margin:14px 0 0 56px}.nls-existing-reply-box input{flex:1;min-width:0;border:1px solid #dbe0e8;border-radius:10px;padding:9px 11px;outline:none}.nls-existing-reply-box input:focus{border-color:#8b5cf6;box-shadow:0 0 0 3px rgba(139,92,246,.08)}.nls-existing-reply-box button{border:0;border-radius:10px;padding:9px 13px;background:#f3e8ff;color:#6d28d9;font-weight:800;cursor:pointer}
      .nls-existing-replies{margin:12px 0 0 56px;border-left:2px solid #f1f5f9;padding-left:13px}.nls-existing-reply{background:#f8fafc;border-radius:11px;padding:10px 12px;margin-top:8px}
      ...
      @media(max-width:640px){.nls-review-card{padding:15px;border-radius:15px}.nls-existing-reply-box,.nls-existing-replies{margin-left:15px}.nls-existing-media-grid img,.nls-existing-media-grid video{width:78px;height:64px}.nls-existing-media-preview{width:74px;height:74px}.nls-review-avatar{width:40px;height:40px}}
      @media(prefers-reduced-motion:reduce){.nls-review-card,.nls-existing-rating-picker button,.nls-existing-media-grid img,.nls-existing-media-grid video{transition:none}}
  ...
  function newFilePreview(files, existingCount=0){
  ...
  function inlineEditor(review){
    ...
    card.querySelector("[data-edit-files]")?.addEventListener("change",e=>{const incoming=Array.from(e.target.files||[]);const current=(review.review_media||[]).filter(m=>!card.querySelector(`[data-existing-media-id="${CSS.escape(m.id)}"]`)===null).length;const displayed=card.querySelectorAll("[data-existing-media-id]").length;const bad=incoming.find(f=>!ALLOWED.has(f.type)||f.size>MAX_SIZE);if(bad){showMessage("Each file must be an allowed image/video and 6MB or smaller.",true,card);e.target.value="";return;}if(displayed+newFiles.length+incoming.length>MAX_FILES){showMessage("You can keep up to 6 photos/videos on a review.",true,card);return;}newFiles.push(...incoming);renderInlineNew(card);e.target.value="";});
  ...
  function replyMarkup(replies,profileMap){return (replies||[]).filter(r=>r.status!=="rejected").map(r=>`<div class="nls-existing-reply"><div class="flex items-center gap-2"><div>${avatarMarkup(profileMap[r.user_id]||{avatar_url:r.avatar_url},r.author_name||"Customer")}</div><div><strong class="text-sm">${esc(r.author_name||"Customer")}</strong><small class="block text-gray-400">${dateText(r.created_at)}</small></div></div><p class="text-sm text-gray-700 mt-2 whitespace-pre-wrap">${esc(r.reply_text)}</p>${mediaMarkup(r.review_media)}</div>`).join("");}
  ...
  async function renderReviews(){
    ...
    try{
      ...
      const cards=visible.length?visible.map((r,i)=>`<div class="nls-review-card" data-review-id="${esc(r.id)}" data-aos="fade-up" data-aos-delay="${i*60}"><div class="flex items-center justify-between mb-3 gap-3"><div class="flex items-center min-w-0">${avatarMarkup(profileMap[r.user_id],r.author_name||"Customer")}<div class="ml-3 min-w-0"><h4 class="font-semibold truncate">${esc(r.author_name||"Customer")}${r.verified_purchase?`<span class="nls-existing-verified"><i class="fas fa-check-circle"></i> Verified Purchase</span>`:""}</h4><div class="flex text-yellow-400 text-sm">${stars(r.rating)}</div></div></div><span class="text-sm text-gray-500 whitespace-nowrap">${dateText(r.created_at)}</span></div><p class="text-gray-700 whitespace-pre-wrap leading-7">${esc(r.review_text)}</p>${mediaMarkup(r.review_media)}<div class="flex items-center gap-4 mt-4"><button type="button" class="text-sm text-gray-500 hover:text-purple-600 font-semibold" data-reply="${esc(r.id)}">Reply</button>${user?.id===r.user_id?`<button type="button" class="text-sm text-gray-500 hover:text-purple-600 font-semibold" data-edit="${esc(r.id)}">Edit</button>`:""}</div><div class="nls-existing-replies">${replyMarkup(r.review_replies,profileMap)}</div><div class="nls-existing-reply-box"><input type="text" maxlength="1000" placeholder="Write a reply…" data-reply-input="${esc(r.id)}"><button type="button" data-send-reply="${esc(r.id)}">Reply</button></div></div>`).join(""):`<div class="text-center py-8"><i class="fas fa-comments text-4xl text-gray-300 mb-4"></i><p class="text-gray-600">No reviews yet. Be the first to review this product!</p></div>`;
